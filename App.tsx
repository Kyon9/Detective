...
const App: React.FC = () => {
  ...
  // 核心变更：以案件 ID 为键，存储每个案件专属的对话和线索
  const [caseHistories, setCaseHistories] = useState<Record<string, CaseHistory>>({});
  ...
  // 派生状态：获取当前激活案件的消息流和证据箱
  const currentMessages = currentCase ? (caseHistories[currentCase.id]?.messages || []) : [];
  const currentClues = currentCase ? (caseHistories[currentCase.id]?.clues || []) : [];

  // 初始化或切换案件逻辑
  const handleSelectCase = (targetCase: Case) => {
    if (caseHistories[targetCase.id]) {
      // 如果已有记录，直接切换当前激活案件，不覆盖历史
  ...
  useEffect(() => {
    checkConnection();
    // 默认开启第一个案件
    if (!currentCase) handleSelectCase(ALL_CASES[0]);
  }, []);
  ...
  const onSaveToSlot = (caseId: string, slot: number) => {
    const history = caseHistories[caseId];
    if (!history) return;
    ...
    const saveData = {
      ...
      clues: history.clues,
      timestamp: Date.now(),
      preview: history.messages.length > 0 ? history.messages[history.messages.length - 1].text.substring(0, 50) : "调查记录"
    };
  ...
  const handleExport = () => {
    if (!currentCase || currentMessages.length === 0) {
  ...
  const handleImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (!data.caseId || !data.messages || !data.clues) throw new Error("格式错误");
        
        const targetCase = ALL_CASES.find(c => c.id === data.caseId);
        if (!targetCase) throw new Error("未知的案件ID");
  ...
  const handleSendMessage = async (e?: React.FormEvent) => {
    ...
    // 仅更新当前案件的消息列表
    setCaseHistories(prev => {
      const history = prev[currentCase.id] || { messages: [], clues: [] };
      return {
        ...prev,
        [currentCase.id]: {
          ...history,
    ...
    try {
      ...
      const response: AgentResponse = await getDetectiveResponse(
        ...
        currentClues.map(c => c.title)
      );
      
      const assistantMessage: Message = {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        text: response.message || "由于现场环境干扰，信号有些模糊。",
        timestamp: Date.now()
      };

      setCaseHistories(prev => {
        const history = prev[currentCase.id];
        const updatedMessages = [...history.messages, assistantMessage];
        let updatedClues = [...history.clues];
        let added = false;
      ...
      const errorMsg = "⚠️ [通讯链路异常] 请检查连接状态。";
      setCaseHistories(prev => {
        const history = prev[currentCase.id];
        return {
          ...prev,
          [currentCase.id]: {
            ...history,
            messages: [...history.messages, { id: 'err-'+Date.now(), role: 'assistant', text: errorMsg, timestamp: Date.now() }]
          }
        };
      });
      setNetStatus('fail');
    ...
    <div className="flex flex-col h-screen overflow-hidden bg-slate-950 text-slate-200">
      <header className="bg-slate-900 border-b border-slate-800 px-10 py-6 flex justify-between items-center shadow-2xl z-20">
        <div className="flex items-center gap-6">
          ...
          <span className="text-base text-slate-500 uppercase tracking-widest font-bold">探案协助系统</span>
        </div>
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <span className="text-sm text-slate-500 uppercase font-black tracking-widest">
              {netStatus === 'ok' ? '连线中' : netStatus === 'testing' ? '同步中' : '连接中断'}
            </span>
          </div>
        </div>
      </header>
      ...
      {solvedSummary && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-xl p-6">
          <div className="bg-slate-900 border-2 border-amber-600 p-10 max-w-3xl rounded shadow-2xl text-center">
            ...
            <button onClick={() => setSolvedSummary(null)} className="px-10 py-4 bg-amber-700 text-white font-bold hover:bg-amber-600 transition-all uppercase tracking-widest text-sm">结案</button>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-1 overflow-hidden relative">
          ...
          <div className="p-6 border-b border-slate-800">
            <div className="flex items-center justify-between mb-6">
              ...
              <button 
                ...
                className={`w-7 h-7 rounded-full flex items-center justify-center border text-sm font-black transition-all ${showActionInfo ? 'bg-amber-700 border-amber-500 text-slate-950' : 'border-slate-700 text-slate-500 hover:border-amber-600 hover:text-amber-600'}`}
              >
                ⓘ
              </button>
            </div>
            ...
            {showActionInfo && (
              <div className="mb-6 p-4 bg-slate-950/80 border border-amber-900/40 rounded shadow-inner animate-fade-in text-xs text-slate-400 font-serif italic leading-relaxed">
                <p className="font-black text-amber-600 uppercase mb-2">存档指南：</p>
                <p>● 保存/读取操作仅针对当前激活案件。</p>
                <p>● 导出文件包含该案件所有已发现线索与对话。</p>
              </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-3 mb-3">
              <button 
                ...
                className={`py-3 text-xs font-black border rounded transition-all uppercase tracking-widest ${currentCase ? 'bg-amber-900/10 border-amber-900/50 text-amber-500 hover:bg-amber-900/20' : 'bg-slate-950 border-slate-800 text-slate-700 cursor-not-allowed'}`}
              >
                📝 保存
              </button>
              <button 
                onClick={() => { setModalMode('load'); setModalOpen(true); }} 
                className="py-3 text-xs font-black border border-slate-800 bg-slate-950 text-slate-500 hover:bg-slate-800 hover:text-white transition-all uppercase tracking-widest"
              >
                📖 读取
              </button>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <button 
                ...
                className={`py-3 text-xs font-black border rounded transition-all uppercase tracking-widest ${currentCase && currentMessages.length > 0 ? 'bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700' : 'bg-slate-950 border-slate-900 text-slate-800 cursor-not-allowed'}`}
              >
                📤 导出
              </button>
              <button 
            ...
            <div className="space-y-4">
              {ALL_CASES.map(c => (
                <div 
                  ...
                  className={`p-6 border rounded cursor-pointer transition-all group relative overflow-hidden ${currentCase?.id === c.id ? 'bg-amber-900/5 border-amber-600/50' : 'bg-slate-950 border-slate-800 hover:border-slate-700'}`}
                >
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">案号：{c.id}</span>
                    {caseHistories[c.id] && (
                      <span className="text-[9px] text-amber-600 font-black uppercase tracking-tighter animate-pulse">● 调查中</span>
                    )}
                  </div>
                  ...
                  <p className="text-sm text-slate-500 mt-3 line-clamp-2 leading-relaxed italic font-serif opacity-80">{c.shortDescription}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-10 space-y-10 custom-scrollbar">
            ...
            {isLoading && (
              <div className="flex justify-start">
                <div className="bg-slate-900/40 p-5 rounded-sm flex items-center gap-6 shadow-lg border-l-4 border-amber-900/50">
                   ...
                   <p className="text-sm text-amber-700 uppercase font-black tracking-[0.4em]">正在整理现场记录</p>
                </div>
              </div>
            )}
          </div>
        ...
        <aside className={`w-96 bg-slate-900 flex flex-col z-20 transition-all duration-700 ${newCluePing ? 'ring-4 ring-amber-600/40 bg-amber-900/20' : ''}`}>
          <ClueBoard clues={currentClues} onSelectClue={setSelectedClue} />
        </aside>
      </div>
